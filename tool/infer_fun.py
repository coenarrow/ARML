...
def create_pseudo_mask(model, dataroot, fm, savepath, n_class, palette, dataset):
    ...
    for iter, (img_name, img_list) in tqdm(enumerate(infer_data_loader),total=len(infer_dataset.object)//1,miniters=10):      
        ...
        ##  Since the white background of images of breast cancer is meaningful (e.g. fat, etc), we do not use it for the training set of BCSS-WSSS.
            ...
            cam_score, _ = infer_utils.dict2npy(cam_dict, label, orig_img, background=None) #此处加入了背景，做修改
            bg_score = np.zeros((1, 224, 224))
            ##  Since the white background of images of breast cancer is meaningful (e.g. fat, etc), we do not use it for the training set of BCSS-WSSS.
            bgcam_score = np.concatenate((cam_score, bg_score), axis=0)
        elif 'brats' in dataset:
            ## in the brats dataset, just copy the version we have for bcss
            cam_score, _ = infer_utils.dict2npy(cam_dict, label, orig_img, background=None)
