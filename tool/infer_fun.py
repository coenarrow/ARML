...
def create_pseudo_mask(model, dataroot, fm, savepath, n_class, palette, dataset):
    ...
    infer_data_loader = DataLoader(infer_dataset,
                                ...
                                num_workers=8,
                                pin_memory=False)
    for iter, (img_name, img_list) in tqdm(enumerate(infer_data_loader),total=len(infer_dataset.object)//1,miniters=10):      
    for iter, (img_name, img_list) in tqdm(enumerate(infer_data_loader),total=len(infer_dataset.object)//1,miniters=10):      
        img_name = img_name[0]
        # print(img_name)
        ...
        if dataset == 'luad':
            cam_score, bg_score = infer_utils.dict2npy(cam_dict, label, orig_img, background='white')
        ...
        ##  Since the white background of images of breast cancer is meaningful (e.g. fat, etc), we do not use it for the training set of BCSS-WSSS.
            ...
            cam_score, bg_score = infer_utils.dict2npy(cam_dict, label, orig_img, background=None) #此处加入了背景，做修改
            ##  Since the white background of images of breast cancer is meaningful (e.g. fat, etc), we do not use it for the training set of BCSS-WSSS.
            cam_score, _ = infer_utils.dict2npy(cam_dict, label, orig_img, None) #此处加入了背景，做修改
            ##  Since the white background of images of breast cancer is meaningful (e.g. fat, etc), we do not use it for the training set of BCSS-WSSS.
            bg_score = np.zeros((1,224,224))
            bgcam_score = np.concatenate((cam_score, bg_score), axis=0)
        elif 'brats' in dataset:
            ## in the brats dataset, we want to ignore the black parts of the image since they're not as useful
            cam_score, bg_score = infer_utils.dict2npy(cam_dict, label, orig_img, background='black')
            cam_score, _ = infer_utils.dict2npy(cam_dict, label, orig_img, None) #此处加入了背景，做修改
            ##  Just copy bcss for now
